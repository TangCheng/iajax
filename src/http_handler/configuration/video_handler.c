...
static void do_get_action_profile(IpcamIAjax *iajax, JsonBuilder *builder, const gchar *name)
{
    GVariant *value = NULL;
    gchar *key = NULL;
    gint i = 0;
    const gchar *profile_member[] =
    {
        "resolution", "frame_rate", "bit_rate", "bit_rate_value", "stream_path"
    };
    json_builder_set_member_name(builder, name);
    json_builder_begin_object(builder);

    for (i = 0; i < ARRAY_SIZE(profile_member); i++)
    {        
        asprintf(&key, "video:%s:%s", name, profile_member[i]);
        value = ipcam_iajax_get_configuration(iajax, key);
        if (value)
        {
            add_value(builder, profile_member[i], value);
...
static gchar* do_get_action(IpcamIAjax *iajax, GList *item_list)
{
    ...
    for (item = g_list_first(item_list); item; item = g_list_next(item))
    {
        ...
        if (g_str_has_suffix(name, "_profile"))
        {
            do_get_action_profile(iajax, builder, name);
        }
        else
        {
            asprintf(&key, "video:%s", name);
            value = ipcam_iajax_get_configuration(iajax, key);
            if (value)
            {
...
START_HANDLER(get_video, HTTP_GET, "/api/1.0/video.json", http_request, http_response, socket)
{
    IpcamIAjax *iajax;
...
START_HANDLER(put_video, HTTP_PUT, "/api/1.0/video.json", http_request, http_response, socket)
{
    ipcam_http_request_handler_do_put_action(put_video,
                                             http_request,
                                             http_response,
                                             socket,
                                             "set_video");
