...
G_DEFINE_TYPE(IpcamHttpEventOutputHandler, ipcam_http_event_output_handler, IPCAM_HTTP_REQUEST_HANDLER_TYPE)
...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_EVENT_OUTPUT_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_event_output", req_node, &res_node);
...
START_HANDLER(get_event_output, HTTP_GET, "/api/1.0/event_output.json", http_request, http_response)
{
    ...
    g_object_get(get_event_output, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_event_output", request, &response);
...
START_HANDLER(put_event_output, HTTP_PUT, "/api/1.0/event_output.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_event_output, "app", &iconfig, NULL);
...
static void ipcam_http_event_output_handler_init(IpcamHttpEventOutputHandler *self)
{
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), get_event_output);
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), put_event_output);
}

static void ipcam_http_event_output_handler_class_init(IpcamHttpEventOutputHandlerClass *klass)
{
}
