...
G_DEFINE_TYPE(IpcamHttpEventCoverHandler, ipcam_http_event_cover_handler, IPCAM_HTTP_REQUEST_HANDLER_TYPE)
...
static void do_get_action_rect(IpcamIAjax *iajax, JsonBuilder *builder, const gchar *name)
{
    ...
    for (i = 0; i < 4; i++)
    {        
        asprintf(&key, "event_cover:%s:rect:%s", name, rect_info[i]);
...
static void do_get_action_schedules(IpcamIAjax *iajax, JsonBuilder *builder, const gchar *name)
{
    ...
    {        
        asprintf(&key, "event_cover:%s:schedules:%s", name, weekday_name[i]);
        value = ipcam_iajax_get_configuration(iajax, key);
        if (value)
        {
...
static gchar* do_get_action(IpcamIAjax *iajax, GList *item_list)
{
    ...
    for (item = g_list_first(item_list); item; item = g_list_next(item))
    {
        ...
        asprintf(&key, "event_cover:%s:enable", name);
        value = ipcam_iajax_get_configuration(iajax, key);
        if (value)
        {
        ...
        asprintf(&key, "event_cover:%s:sensitivity", name);
        value = ipcam_iajax_get_configuration(iajax, key);
        if (value)
        {
...
START_HANDLER(get_event_cover, HTTP_GET, "/api/1.0/event_cover.json", http_request, http_response, socket)
{
    IpcamIAjax *iajax;
    ...
    g_object_get(get_event_cover, "app", &iajax, NULL);
...
START_HANDLER(put_event_cover, HTTP_PUT, "/api/1.0/event_cover.json", http_request, http_response, socket)
{
    ipcam_http_request_handler_do_put_action(put_event_cover,
                                             http_request,
                                             http_response,
                                             socket,
                                             "set_event_cover");
...
static void ipcam_http_event_cover_handler_init(IpcamHttpEventCoverHandler *self)
{
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), get_event_cover);
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), put_event_cover);
}

static void ipcam_http_event_cover_handler_class_init(IpcamHttpEventCoverHandlerClass *klass)
{
}
