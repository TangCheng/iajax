...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_EVENT_COVER_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_event_cover", req_node, &res_node);
...
START_HANDLER(get_event_cover, HTTP_GET, "/api/1.0/event_cover.json", http_request, http_response)
{
    ...
    g_object_get(get_event_cover, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_event_cover", request, &response);
...
START_HANDLER(put_event_cover, HTTP_PUT, "/api/1.0/event_cover.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_event_cover, "app", &iconfig, NULL);
