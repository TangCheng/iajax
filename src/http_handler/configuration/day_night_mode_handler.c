...
G_DEFINE_TYPE(IpcamHttpDayNightModeHandler, ipcam_http_day_night_mode_handler, IPCAM_HTTP_REQUEST_HANDLER_TYPE)
...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_DAY_NIGHT_MODE_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_day_night_mode", req_node, &res_node);
...
START_HANDLER(get_day_night_mode, HTTP_GET, "/api/1.0/day_night_mode.json", http_request, http_response)
{
    ...
    g_object_get(get_day_night_mode, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_day_night_mode", request, &response);
...
START_HANDLER(put_day_night_mode, HTTP_PUT, "/api/1.0/day_night_mode.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_day_night_mode, "app", &iconfig, NULL);
...
static void ipcam_http_day_night_mode_handler_init(IpcamHttpDayNightModeHandler *self)
{
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), get_day_night_mode);
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), put_day_night_mode);
}

static void ipcam_http_day_night_mode_handler_class_init(IpcamHttpDayNightModeHandlerClass *klass)
{
}
