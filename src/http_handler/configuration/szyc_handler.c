...
G_DEFINE_TYPE(IpcamHttpSzycHandler, ipcam_http_szyc_handler, IPCAM_HTTP_REQUEST_HANDLER_TYPE)
...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_SZYC_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_szyc", req_node, &res_node);
...
START_HANDLER(get_szyc, HTTP_GET, "/api/1.0/szyc.json", http_request, http_response)
{
    ...
    g_object_get(get_szyc, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_szyc", request, &response);
...
START_HANDLER(put_szyc, HTTP_PUT, "/api/1.0/szyc.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_szyc, "app", &iconfig, NULL);
...
static void ipcam_http_szyc_handler_init(IpcamHttpSzycHandler *self)
{
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), get_szyc);
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), put_szyc);
}

static void ipcam_http_szyc_handler_class_init(IpcamHttpSzycHandlerClass *klass)
{
}
