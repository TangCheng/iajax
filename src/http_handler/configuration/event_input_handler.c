...
static void do_get_action_schedules(IpcamIAjax *iajax, JsonBuilder *builder, const gchar *name)
{
    ...
    {        
        asprintf(&key, "event_input:%s:schedules:%s", name, weekday_name[i]);
...
static gchar* do_get_action(IpcamIAjax *iajax, GList *item_list)
{
    ...
    for (item = g_list_first(item_list); item; item = g_list_next(item))
    {
        ...
        asprintf(&key, "event_input:%s:enable", name);
        value = ipcam_iajax_get_configuration(iajax, key);
        if (value)
        {
...
START_HANDLER(get_event_input, HTTP_GET, "/api/1.0/event_input.json", http_request, http_response, socket)
{
    IpcamIAjax *iajax;
    ...
    g_object_get(get_event_input, "app", &iajax, NULL);
...
START_HANDLER(put_event_input, HTTP_PUT, "/api/1.0/event_input.json", http_request, http_response, socket)
{
    ipcam_http_request_handler_do_put_action(put_event_input,
                                             http_request,
                                             http_response,
                                             socket,
                                             "set_event_input");
