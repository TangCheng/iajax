...
G_DEFINE_TYPE(IpcamHttpEventInputHandler, ipcam_http_event_input_handler, IPCAM_HTTP_REQUEST_HANDLER_TYPE)
...
static gchar* do_get_action(IpcamIConfig *iconfig, GList *item_list)
{
    ...
    IpcamMessageHandler *msg_handler = g_object_new(IPCAM_TYPE_EVENT_INPUT_MSG_HANDLER,
                                                      "app", iconfig, NULL);

    ipcam_message_handler_do_get(msg_handler, "get_event_input", req_node, &res_node);
...
START_HANDLER(get_event_input, HTTP_GET, "/api/1.0/event_input.json", http_request, http_response)
{
    ...
    g_object_get(get_event_input, "app", &iconfig, NULL);
...
static gchar* do_put_action(IpcamIConfig *iconfig, JsonNode *request)
{
    ...
    ipcam_message_handler_do_put(msg_handler, "set_event_input", request, &response);
...
START_HANDLER(put_event_input, HTTP_PUT, "/api/1.0/event_input.json", http_request, http_response)
{
    gchar *body = NULL;
    IpcamIConfig *iconfig;
    gboolean success = FALSE;

    g_object_get(put_event_input, "app", &iconfig, NULL);
...
static void ipcam_http_event_input_handler_init(IpcamHttpEventInputHandler *self)
{
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), get_event_input);
    ipcam_http_request_handler_register(IPCAM_HTTP_REQUEST_HANDLER(self), put_event_input);
}

static void ipcam_http_event_input_handler_class_init(IpcamHttpEventInputHandlerClass *klass)
{
}
