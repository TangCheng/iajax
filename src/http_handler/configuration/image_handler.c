...
START_HANDLER(get_image, HTTP_GET, "/api/1.0/image.json", http_request, http_response, socket)
{
    IpcamIAjax *iajax;
    ...
    g_object_get(get_image, "app", &iajax, NULL);
...
START_HANDLER(put_image, HTTP_PUT, "/api/1.0/image.json", http_request, http_response, socket)
{
    ipcam_http_request_handler_do_put_action(put_image,
                                             http_request,
                                             http_response,
                                             socket,
                                             "set_image");
